...
export default class View {
  ...
  renderGarage(cars: CarModel[], carsPage: number) {
    ...
    <h2>Page #${carsPage}</h2>
    <ul class="garage">
      ${cars.map((car: CarModel) => `<li>${this.renderCar(car)}</li>`).join('')}
    </ul>`;
  }
  ...
  renderCarButton(car: CarModel) {
    ...
    selectButton.setAttribute('id', `select-car-${car.id}`);
    selectButton.textContent = 'Select';

    const removeButton = document.createElement('button');
    removeButton.classList.add('button', 'remove-button');
    removeButton.setAttribute('id', `remove-car-${car.id}`);
    ...
    carButtons.addEventListener('click', async (e) => this.garage.selectCar(e));
    return carButtons;
  }

  renderCar(car: CarModel) {
    /*
    ...
    flag.setAttribute('id', `flag-${car.id}`);

    road.appendChild(carWrapper);
    ...
    startEngineButton.classList.add('icon', 'start-engine-button');
    startEngineButton.setAttribute('id', `start-car-${car.id}`);
    startEngineButton.textContent = 'A';

    const stopEngineButton = document.createElement('button');
    stopEngineButton.classList.add('icon', 'stop-engine-button');
    stopEngineButton.setAttribute('id', `stop-car-${car.id}`);
    stopEngineButton.textContent = 'B';

    controlPanel.appendChild(startEngineButton);
    controlPanel.appendChild(stopEngineButton);

    const carElement = document.createElement('div');
    carElement.classList.add('car');
    carElement.setAttribute('id', `car-${car.id}`);
    carElement.innerHTML = this.renderCarImage(car.color);
